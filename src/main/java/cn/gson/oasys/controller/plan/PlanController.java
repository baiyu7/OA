...
public class PlanController {
    ...
    private void plantablepaging(HttpServletRequest request, Model model, HttpSession session, int page,
        ...
        Page<User> uListpage = null;
        if (role.getRoleId() == 2) {
            uListpage = userService.findmyemployuser(page, baseKey, -2);
        } else if (role.getRoleId() == 3) {
            uListpage = userService.findmyemployuser(page, baseKey, -1);
        } else {
            uListpage = userService.findmyemployuser(page, baseKey, userid);
        }
