...
public class AttendceController {
    ...
    private void weektablepaging(HttpServletRequest request, HttpSession session, int page, String baseKey) {
        ...
        if (role.getRoleId() == 2) {
            userspage = userService.findmyemployuser(page, baseKey, -2);
        } else if (role.getRoleId() == 3) {
            userspage = userService.findmyemployuser(page, baseKey, -1);
        } else {
            userspage = userService.findmyemployuser(page, baseKey, userId);
        }
