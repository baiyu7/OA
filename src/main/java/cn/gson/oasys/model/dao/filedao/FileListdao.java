...
public interface FileListdao extends PagingAndSortingRepository<FileList, Long> {
    ...
    @Query("from FileList f where f.user=?1 and f.fileIstrash=0 and f.type LIKE '%document%'")
    List<FileList> finddocument(User user);

    @Query("from FileList f where f.user=?1 and f.fileIstrash=0 and f.type LIKE '%picture%'")
    List<FileList> findpicture(User user);

    @Query("from FileList f where f.user=?1 and f.fileIstrash=0 and f.type LIKE '%music%'")
    List<FileList> findmusic(User user);

    @Query("from FileList f where f.user=?1 and f.fileIstrash=0 and f.type LIKE '%video%'")
    List<FileList> findvideo(User user);

    @Query("from FileList f where f.user=?1 and f.fileIstrash=0 and f.type LIKE '%yasuo%'")
    List<FileList> findyasuo(User user);
